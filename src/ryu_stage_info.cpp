...
animation_name_location_map populate_rocky_stage_info()
{
    animation_name_location_map rocky_stage_location_map;
    std::string stage_floor = "ROCKY_STAGE_FLOOR";
    sprite_quad_locations stage_floor_quads;
    stage_floor_quads.insert(std::make_pair<unsigned int, pixel_sprite_info>(0, {{9,21}, {384,107}, 0.0f}));
    rocky_stage_location_map[stage_floor] = stage_floor_quads;

    std::string stage_background = "ROCKY_STAGE_BACKGROUND";
    sprite_quad_locations stage_backbround_quads;
    stage_backbround_quads.insert(std::make_pair<unsigned int, pixel_sprite_info>(0, {{35,329}, {320,136}, 0.0f}));
    rocky_stage_location_map[stage_background] = stage_backbround_quads;

    return rocky_stage_location_map;
}
