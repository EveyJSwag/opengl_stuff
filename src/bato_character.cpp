...
{
    bato_character_sprite_anim->set_flip_anim(true);
}
...
{
    keyboard_ref = k;
    bato_character_name = character_name;
    bato_character_location = character_location;
    character_animation_map = a_animation_info;
    bato_character_speed = character_speed;
    char_base_width = base_width;
    bato_character_sprite_anim = std::make_unique<sprite_animator>(
        sprite_animator(
            sprite_sheet_name, 
            character_location, 
...
void bato_character::handle_character()
{
    ...
    {
        bato_character_location.x += move_factor;
        bato_character_sprite_anim->move_sprite_x(move_factor);
    }
    */
    bato_character_sprite_anim->do_animation("BATO_IDLE", 8);
    /*
    if (bato_character_sprite_anim->get_current_frame() == character_animation_map[curr_anim_string].size() - 1 &&
...
bato_character::action_types bato_character::process_inputs()
{
    int none_count = 0;
    ...
    if (keyboard_ref->get_input_buffer()[keyboard_ref->get_input_buffer_size() - 1] != keyboard::NONE)
    {
        ...
        switch (keyboard_ref->get_button(curr_input))
        {
            case (HP_BIT):
                ...
                if (is_special_move_dpf(HP_BIT))
                {
                    ...
                    if (bato_character_sprite_anim->get_flip_anim()) {speed_factor*=-1.0f;}
        ...
        switch (keyboard_ref->get_direction(curr_input))
        {
            ...
            case (UP_BIT):
                ...
                break;
            case (LEFT_BIT):
                bato_character_location.x -= 0.01f;
                bato_character_sprite_anim->move_sprite_x(-0.01f);
                return WALK_BACK;
                break;
            case (RIGHT_BIT):
                bato_character_location.x -= 0.01f;
